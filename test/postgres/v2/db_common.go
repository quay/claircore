...
// TestMatcherDB returns a [pgxpool.Config] for a configured Matcher database.
//
// If any errors are encountered, the test is failed and exited.
func TestMatcherDB(ctx context.Context, t testing.TB) *pgxpool.Config {
	return testDB(ctx, t, dbMatcher)
}

// TestIndexerDB returns a [pgxpool.Config] for a configured Indexer database.
//
// If any errors are encountered, the test is failed and exited.
func TestIndexerDB(ctx context.Context, t testing.TB) *pgxpool.Config {
...
func testDB(ctx context.Context, t testing.TB, which dbFlavor) *pgxpool.Config {
	t.Helper()
	...
	ccfg := cfg.ConnConfig.Copy()
	ccfg.DefaultQueryExecMode = pgx.QueryExecModeExec
	mdb := stdlib.OpenDB(*ccfg)
	defer mdb.Close()
	// run migrations
