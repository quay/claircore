...
func main() {
	...
	err = Main(ctx, set.Args())
}

// Main is the entrypoint for search.
func Main(ctx context.Context, qs []string) error {
	c := &http.Client{}
	hc := &HydraClient{c}
	cc := &CatalogClient{c}
	runTime := time.Now()
	...
	eg.Go(func() error {
		...
		for src := range output {
			if !first {
				io.WriteString(os.Stdout, "\x1C") // write ␜ if outputting multiple txtars
			}
			_, err := io.Copy(os.Stdout, bytes.NewReader(txtar.Format(src)))
			if err != nil {
