...
func New(ctx context.Context, r io.ReaderAt, size int64, buf *os.File) (*FS, error) {
	...
	sys := new(FS)
	err := sys.new(ctx, r, size, buf)
	if err != nil {
		span.SetStatus(codes.Error, "unsuccessful tarfs creation")
		if sys.cleanup != nil {
			err = errors.Join(err, sys.cleanup.Close())
		}
	...
	runtime.SetFinalizer(sys, func(_ *FS) {
		panic(fmt.Sprintf("%s:%d: FS not closed", file, line))
	})
	profile.Add(weak.Make(sys), 1)
	return sys, nil
}

func (sys *FS) new(ctx context.Context, r io.ReaderAt, size int64, buf *os.File) error {
	seekable := false
	...
	if size < 0 {
		...
		case interface{ Stat() (fs.FileInfo, error) }:
			...
			if err != nil {
				return fmt.Errorf("tarfs: unable to stat: %w", err)
			}
			size = fi.Size()
		case io.Seeker:
			var err error
			...
			if err != nil {
				return fmt.Errorf("tarfs: unable to seek: %w", err)
			}
			if _, err := v.Seek(0, io.SeekStart); err != nil {
				return fmt.Errorf("tarfs: unable to seek: %w", err)
			}
		default:
			return errors.New("tarfs: unable to determine size of ReaderAt")
		}
	}
	...
	default:
		return fmt.Errorf("tarfs: unable to read footer: %w", err)
	}
	...
	default:
		...
		default:
			return fmt.Errorf("tarfs: unable to read header: %w", err)
		}
		var unz bool
	Loop:
		switch {
		...
		if err != nil {
			return fmt.Errorf("tarfs: error examining standard tar: %w", err)
		}
		...
		if err != nil {
			return fmt.Errorf("tarfs: unable to build TOC: %w", err)
		}
		err = sys.init(buf, toc.Entry, inodeIdent)
	}
	if err != nil {
		return fmt.Errorf("tarfs: error initializing FS: %w", err)
	}
	if toc.Version != 1 {
	...
	if seekable {
		if buf == nil {
			return ErrFileNeeded
		}
		d, err := newDiskBuf(r, dec, sys.inodeByIdx, buf)
		if err != nil {
			return fmt.Errorf("tarfs: unable to create disk buffer: %w", err)
		}
		sys.cleanup = d
...
func (sys *FS) Close() error {
	...
	profile.Remove(weak.Make(sys))
	if sys.cleanup != nil {
		return sys.cleanup.Close()
	}
	return nil
}
