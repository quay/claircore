...
func (u *Updater) Run(ctx context.Context, strict bool) error {
	...
	wg.Add(lim)
	feed, errCh := make(chan driver.Updater), make(chan error)
	var errs []error
...
func (u *Updater) updaters(ctx context.Context, cfg driver.Configs) ([]driver.Updater, error) {
...
func (u *Updater) parseOne(ctx context.Context, upd driver.Updater, in fs.FS) (*parseResult, error) {
	var (
		any  bool
		res  parseResult
		err  error
...
func (u *Updater) fetchAndParse(ctx context.Context, spool *os.File, pfps map[string]driver.Fingerprint, upd driver.Updater) error {
	spoolname := spool.Name()
	var name string
	pprof.Do(ctx, pprof.Labels("task", "updater_name"), func(_ context.Context) {
		name = upd.Name()
...
// Context's timeout.
func feeder(ctx context.Context, ch chan<- driver.Updater, us []driver.Updater) func() error {
	return func() error {
		defer close(ch)
		for _, u := range us {
			select {
