...
// UpdateE2e implements a multi-phase test ensuring an update operation and
// diff works end to end.
type updateE2e struct {
	Name    string
	Insert  int
	Updates int
...
func (e *updateE2e) Run(ctx context.Context) func(*testing.T) {
	...
	return func(t *testing.T) {
		...
		var err error
		e.pool, err = pgxpool.NewWithConfig(ctx, cfg)
		if err != nil {
			t.Fatal(err)
		}
		store, err := NewMatcherV1(ctx, cfg)
		if err != nil {
			t.Fatal(err)
		}
		e.s = store
...
func (e *updateE2e) vulns() [][]*claircore.Vulnerability {
...
func (e *updateE2e) calcDiff(i int) [2][]*claircore.Vulnerability {
	if i >= e.Updates {
...
func (e *updateE2e) DeleteUpdateOperations(ctx context.Context) func(*testing.T) {
	return func(t *testing.T) {
		const (
