...
func TestDeleteManifests(t *testing.T) {
	...
	t.Run("One", func(t *testing.T) {
		...
		if _, err := pool.Exec(ctx, insertManifest, want); err != nil {
			t.Error(err)
		}
	...
	t.Run("Subset", func(t *testing.T) {
		...
		if _, err := pool.Exec(ctx, insertManifest, in); err != nil {
			t.Error(err)
		}
...
INSERT INTO manifest_layer (i, manifest_id, layer_id)
	...
	t.Run("Layers", func(t *testing.T) {
		...
		if _, err := pool.Exec(ctx, insertManifest, ms); err != nil {
			t.Error(err)
		}
		if _, err := pool.Exec(ctx, insertLayers, ls); err != nil {
			t.Error(err)
		}
		var nLayers int
		...
		for i, m := range ms {
			tag, err := pool.Exec(ctx, assoc, ls[i:i+layersPer], m)
	...
	t.Run("ManifestIndex", func(t *testing.T) {
		const (
			layersN    = 4
			manifestsN = 100
			packageN   = 10
		...
		for i := 0; i < manifestsN; i++ {
			...
			tag, err := pool.Exec(ctx, assoc, ls, ir.Hash)
			t.Logf("affected: %d", tag.RowsAffected())
			if err != nil {
				t.Error(err)
			}
			...
			if err := store.RegisterScanners(ctx, indexer.VersionedScanners{scnr}); err != nil {
				t.Error(err)
			}

			pkgs := test.GenUniquePackages(packageN)
			layer := &claircore.Layer{Hash: ls[0]}
			if err := store.IndexPackages(ctx, pkgs, layer, scnr); err != nil {
				t.Error(err)
			}
			// Retrieve packages from DB so they are all correctly ID'd
			if pkgs, err = store.PackagesByLayer(ctx, layer.Hash, []indexer.VersionedScanner{scnr}); err != nil {
				t.Error(err)
			}
			...
			if err := store.IndexManifest(ctx, ir); err != nil {
				t.Error(err)
			}
		}
