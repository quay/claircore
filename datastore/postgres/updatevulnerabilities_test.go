...
// SharedAliasName is attached to every vulnerability generated by
// genAliasVulns, so it recurs in every flush chunk.
const sharedAliasName = "GHSA-shared-0000-0000"

func genAliasVulns(updater string, n int) []*claircore.Vulnerability {
	shared := claircore.Alias{Space: unique.Make("GHSA"), Name: sharedAliasName}
	...
	for i := range vulns {
		...
		vulns[i] = &claircore.Vulnerability{
			...
			Name:    name,
			Package: &claircore.Package{Name: "test-pkg"},
			Self:    claircore.Alias{Space: unique.Make("CVE"), Name: name},
			Aliases: []claircore.Alias{
				shared,
				{Space: unique.Make("GHSA"), Name: "GHSA-" + name},
			},
		}
	}
	return vulns
...
func TestUpdateVulnerabilitiesChunked(t *testing.T) {
	integration.NeedDB(t)
	ctx := test.Logging(t)
	...
	// The insert batch flushes every 500 vulnerabilities (two queued queries
	// per vulnerability, flushed at 1000), so 1200 crosses two chunk
	// boundaries and leaves a remainder for the final flush.
	const vulnCt = 1200
	vulns := genAliasVulns(t.Name(), vulnCt)

	if _, err := store.UpdateVulnerabilities(ctx, t.Name(), driver.Fingerprint(uuid.New().String()), vulns); err != nil {
		t.Fatalf("UpdateVulnerabilities: %v", err)
	}
	...
	checks := []struct {
		...
		{"vuln rows", `SELECT count(*) FROM vuln WHERE updater = $1`, vulnCt},
		{"self links", `SELECT count(*) FROM vulnerability_self s JOIN vuln v ON s.vulnerability = v.id WHERE v.updater = $1`, vulnCt},
		// One shared alias plus one unique alias per vulnerability.
		{"alias links", `SELECT count(*) FROM vulnerability_alias a JOIN vuln v ON a.vulnerability = v.id WHERE v.updater = $1`, 2 * vulnCt},
	}
	for _, c := range checks {
		var got int
		if err := pool.QueryRow(ctx, c.query, t.Name()).Scan(&got); err != nil {
			t.Fatalf("counting %s: %v", c.desc, err)
		}
		if got != c.want {
	...
	// The shared alias row is created during the first chunk; vulnerabilities
	// in later chunks must still link to it.
	var sharedCt int
	err := pool.QueryRow(ctx, `
		SELECT count(*)
		FROM vulnerability_alias va
			JOIN vuln v ON va.vulnerability = v.id
			JOIN alias a ON va.alias = a.id
		WHERE v.updater = $1 AND a.name = $2`, t.Name(), sharedAliasName).Scan(&sharedCt)
	if err != nil {
		t.Fatalf("counting shared alias links: %v", err)
	}
	if sharedCt != vulnCt {
		t.Errorf("shared alias links: got %d, want %d", sharedCt, vulnCt)
	}
}
