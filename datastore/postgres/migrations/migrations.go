...
// MigrationFunc is some user-provided code to finish a migration.
//
// The passed [pgx.Conn] is not reused between different MigrationFuncs and is
// simply closed after the function returns. This means the function should
// return an explicit error and check for invalid states, as the migrator does
// not check connection state.
...
func runMigrations(ctx context.Context, cfg *pgx.ConnConfig, table pgx.Identifier, sys fs.FS) error {
	...
	ms := make([]migration, len(ents))
	for i, ent := range ents {
		ms[i] = newMigration(ent.Name())
	}
