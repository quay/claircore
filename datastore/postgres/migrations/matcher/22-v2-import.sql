...
SET
  LOCAL search_path TO matcher_v2_import,
  "$user",
  public;
...
CREATE TYPE reference AS (namespace TEXT, name TEXT, uri TEXT[]);
...
CREATE TYPE attr AS (mediatype TEXT, data JSONB);
...
-- }}}
CREATE FUNCTION array_distinct (a anyarray) RETURNS anyarray LANGUAGE SQL IMMUTABLE STRICT AS $$
	SELECT
...
-- {{{ Run management
CREATE OR REPLACE FUNCTION start_run (ref UUID) RETURNS BIGINT LANGUAGE plpgsql STRICT AS $$
DECLARE
u BIGINT;
BEGIN
	INSERT INTO matcher_v2.run (ref) VALUES (ref) RETURNING id INTO STRICT u;
	RETURN u;
END;
$$;
...
CREATE OR REPLACE FUNCTION start_updater_run (ref UUID, upd BIGINT, run BIGINT) RETURNS BIGINT LANGUAGE plpgsql STRICT AS $$
DECLARE
ur BIGINT;
BEGIN
...
CREATE OR REPLACE FUNCTION finish_updater_run (upd_run BIGINT, fp JSONB, err TEXT) RETURNS void LANGUAGE plpgsql AS $$
BEGIN
	UPDATE
...
CREATE OR REPLACE FUNCTION finish_run (run_id BIGINT) RETURNS void LANGUAGE plpgsql STRICT AS $$
BEGIN
	UPDATE matcher_v2.run SET complete = TRUE WHERE id = run_id;
END;
$$;
...
-- }}}
-- {{{ Stage
CREATE OR REPLACE PROCEDURE stage () LANGUAGE plpgsql AS $$
BEGIN
	...
	ELSE
		CREATE TEMPORARY TABLE advisory_import OF matcher_v2_import.advisory_import_row ON COMMIT DROP;
...
-- }}}
-- {{{ Commit_add
CREATE OR REPLACE PROCEDURE commit_add (run_id BIGINT, upd_id BIGINT, upd_run_id BIGINT) LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
	...
	WHEN MATCHED AND
		ROW (src.issued, src.summary, src.description, src.uri, src.severity, src.normalized_severity)
		IS NOT DISTINCT FROM
			ROW (tgt.issued, tgt.summary, tgt.description, tgt.uri, tgt.severity, tgt.normalized_severity) THEN
			DO NOTHING
	WHEN MATCHED THEN
		UPDATE SET
	...
	USING (
			SELECT namespace, name, array_agg(uri) FILTER (WHERE cardinality(uri) = 0) AS uri
			FROM (SELECT (unnest(reference)).* FROM advisory_import)
	...
	WHEN MATCHED AND src.uri = tgt.uri THEN
		DO NOTHING
	WHEN MATCHED THEN
		UPDATE SET
...
-- }}}
-- {{{ Commit_snapshot
CREATE OR REPLACE PROCEDURE commit_snapshot (run_id BIGINT, upd_id BIGINT, upd_run_id BIGINT) LANGUAGE plpgsql AS $$
DECLARE
	ev JSONB := jsonb_build_object('run', run_id, 'updater', upd_id, 'update_run', upd_run_id);
BEGIN
...
-- }}}
-- {{{ Commit_remove
CREATE OR REPLACE PROCEDURE commit_remove (run_id BIGINT, upd_id BIGINT, upd_run_id BIGINT) LANGUAGE plpgsql AS $$
DECLARE
	ev JSONB := jsonb_build_object('run', run_id, 'updater', upd_id, 'update_run', upd_run_id);
BEGIN
	...
	WHERE id IN (
			...
			FROM
				...
				JOIN advisory_import AS i ON a.name = (i.advisory).name
			WHERE
				updater = upd_id);
...
-- }}}
-- {{{ Finish
CREATE OR REPLACE PROCEDURE finish (run_id BIGINT, upd_id BIGINT, upd_run_id BIGINT) LANGUAGE plpgsql AS $$
DECLARE
	ev JSONB;
BEGIN
