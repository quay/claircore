...
type Encoder struct {
	...
	// The PURL registry to use.
	PURLRegistry purl.PURLRegistry
}
...
func WithPURLRegistry(registry purl.PURLRegistry) Option {
	return func(e *Encoder) {
		e.PURLRegistry = registry
	}
}
...
func (e *Encoder) parseIndexReport(ctx context.Context, ir *claircore.IndexReport) (*v2_3.Document, error) {
	...
	for _, r := range ir.IndexRecords() {
		...
		if !ok {
			...
			if e.PURLRegistry != nil {
				purl, err := e.PURLRegistry.Generate(ctx, r)
				switch {
				case err != nil:
