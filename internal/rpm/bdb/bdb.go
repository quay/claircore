...
func (db *PackageDB) getPage(n uint32) *io.SectionReader {
	return io.NewSectionReader(db.r, int64(n)*int64(db.m.PageSize), int64(db.m.PageSize))
}

// All returns an [io.ReaderAt] for each rpm header in the PackageDB.
func (db *PackageDB) All(ctx context.Context) (iter.Seq[io.ReaderAt], func() error) {
	var final error

	// Returned iterator.
	seq := func(yield func(io.ReaderAt) bool) {
		for n, lim := uint32(0), db.m.LastPageNo+1; n < lim; n++ {

			// First, read in the hash page.
			var h hashpage
			pg := db.getPage(n)
			if err := binary.Read(pg, db.ord, &h); err != nil {
				final = fmt.Errorf("bdb: error reading hashpage: %w", err)
				return
			}
			// Skip pages of "incorrect" types.
			if h.Type != pagetypeHashUnsorted && h.Type != pagetypeHash {
				continue
			}
			// Assert it's well-formed.
			if h.Entries%2 != 0 {
				final = errors.New("bdb: odd number of entries")
				return
			}
			...
			for i := range ent {
				if err := binary.Read(pg, db.ord, &ent[i]); err != nil {
					final = fmt.Errorf("bdb: error reading hash entry: %w", err)
					return
				}
			}
			...
			for _, e := range ent {
				...
				if _, err := view.ReadAt(kind, 0); err != nil {
					final = fmt.Errorf("bdb: error peeking page type: %w", err)
					return
				}
				if kind[0] != pagetypeHashOffIndex {
					continue
				}
				// Read the page header, now that we know it's correct.
				var offpg hashoffpage
				if err := binary.Read(view, db.ord, &offpg); err != nil {
					final = fmt.Errorf("bdb: error reading hashoffpage: %w", err)
					return
				}

				// This rope will hold the section readers that get logically
				// concatenated into one view of the bytes.
				var r rope
				...
				for n := offpg.PageNo; n != 0; {
					...
					if h.NextPageNo == 0 {
						...
						data = io.NewSectionReader(db.r, off, int64(db.m.PageSize)-hashpageSize)
					}
					if err := r.add(data); err != nil {
						final = fmt.Errorf("bdb: error adding to rope: %w", err)
						return
					}
					n = h.NextPageNo
				}
...
// NOTE(hank) The rope implementation in this package could be updated to
// (theoretically) cause less GC pressure by not eagerly constructing
// a [io.SectionReader] for every segment.
