...
func NewAnnotator(ctx context.Context, sys fs.FS) (Annotator, error) {
	...
	var a annotator
	if err := a.Init(ctx, r, enum); err != nil {
		return nil, err
	}
	return &a, nil
}
...
// Init initializes the book-keeping for producing multiple independent mapping
// iterators for a given dnf history database.
func (a *annotator) Init(ctx context.Context, r fs.File, enum int) (err error) {
	a.absent.Clear()
	a.repo.Clear()
	a.removedEnum = enum
...
// FindDatabase reports the path for the dnf history database and the "removed"
// enum to use, if found. If there's not a dnf history database at the
// well-known locations, [errNoDatabase] is reported.
...
// ErrNoDatabase is a sentinel error returned by [findDatabase].
