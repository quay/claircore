...
func (r *Reader) ReadAt(b []byte, off int64) (int, error) {
	...
	case http.StatusRequestedRangeNotSatisfiable:
		return 0, errWithStatus(srvBotch(req.Header.Get(`range`)), res.StatusCode)
	default:
		return 0, errStatus(res.StatusCode)
	}
...
func rangeUnsupported(s string) error {
	...
	return &RangeUnsupported{URL: u}
}

var _ error = (*RangeUnsupported)(nil)

// RangeUnsupported is reported when Range requests are unsupported by the
// indicated resource.
type RangeUnsupported struct{ URL *url.URL }

func (e *RangeUnsupported) Error() string {
	return fmt.Sprintf("httpreader: origin %q does not support range requests (resource: %q)", e.URL.Host, e.URL.Path)
}
...
Rangefind:
	...
	ceil := int(math.Ceil(math.Log2(float64(nBuckets))))
	attrs := []slog.Attr{
	...
	if reqs > lim {
		return errors.New("httpreader: rangefinding loop")
	}
...
func (s *statusError) Error() string {
	if s.Inner == nil {
		return fmt.Sprintf("httpreader: unexpected status: %d %s", s.Status, http.StatusText(s.Status))
	}
	return fmt.Sprintf("httpreader: unexpected status (%d): %v", s.Status, s.Inner)
}

func (s *statusError) Unwrap() error { return s.Inner }

func srvBotch(r string) error {
	return fmt.Errorf("server botch (Range: %q)", r)
}

func srvContentBotch(res *http.Response) error {
	return fmt.Errorf("origin %q header botch (Content-Range: %q)", res.Request.URL.Host, res.Header.Get(`content-range`))
}
