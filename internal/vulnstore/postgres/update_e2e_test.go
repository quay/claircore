...
func (e *e2e) update(t *testing.T) {
	...
	for i := e.updateN - 1; i >= 0; i-- {
		...
		err := e.s.UpdateVulnerabilities(e.ctx, e.updater, uoid, fp, vulns)
		if err != nil {
		...
		checkInsertedVulns(t, e.db, uoid, vulns)
	}
	e.updateOps = stack
...
func (e *e2e) getUpdateOperations(t *testing.T) {
	...
	out, err := e.s.GetUpdateOperations(e.ctx, []string{e.updater})
	if err != nil {
	...
	for i := 0; i > e.updateN; i++ {
		...
		if expected.ID != retrieved.ID {
			t.Errorf("expected %v but got %v for ID", expected.ID, retrieved.ID)
		}
...
func (e *e2e) diff(t *testing.T) {
	...
	uoid := uuid.New().String()
	fp := driver.Fingerprint(uuid.New().String())
	err := e.s.UpdateVulnerabilities(e.ctx, e.updater, uoid, fp, insertedVulns)
	if err != nil {
	...
	diff, err := e.s.GetUpdateOperationDiff(e.ctx, a.ID, b.ID)
	if err != nil {
...
func (e *e2e) deleteUpdateOperations(t *testing.T) {
	...
	for _, UO := range e.updateOps {
		err := e.s.DeleteUpdateOperations(e.ctx, []string{UO.ID})
		if err != nil {
			t.Fatalf("failed to get delete UpdateOperation: %v", err)
		}
		checkDeletion(t, e.db, UO.ID)
	}
}
