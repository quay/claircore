...
func TestError(t *testing.T) {
	t.Run("Unsupported", func(t *testing.T) {
		err := &loadError{
			LibC:       errors.New("libc"),
			LibGuestFS: errors.New("libguestfs"),
		}

		t.Logf("%q == %q", err, errors.ErrUnsupported)
		if !errors.Is(err, errors.ErrUnsupported) {
			t.Fail()
		}
	})
	t.Run("Unwrap", func(t *testing.T) {
		errLibC := errors.New("libc")
		err := &loadError{
			LibC:       errLibC,
			LibGuestFS: errors.New("libguestfs"),
		}

		t.Logf("%q == %q", err, errLibC)
		if !errors.Is(err, errLibC) {
			t.Fail()
		}
	})
	...
	t.Run("Plural", func(t *testing.T) {
		...
		got := err.Error()
		want := "load errors: libc, libguestfs"
...
func TestLoad(t *testing.T) {
	...
	// BUG(hank) The directory walking doesn't work correctly.
	fs.WalkDir(sys, "usr", func(p string, ent fs.DirEntry, err error) error {
		info, err := ent.Info()
		if err != nil {
			t.Error(err)
			return err
